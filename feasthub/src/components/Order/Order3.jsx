    ...
    const fetchOrderData = async () => {
      try {
        ...
        if (orderSnapshot.exists()) {
          ...
          // Fetch menu data based on seller's email
          const sellerEmail = orderData.Email; // Assuming seller's email is stored in orderData
          const menuRef = doc(db, "SellerMenu", sellerEmail);
          ...
          if (menuSnapshot.exists()) {
            ...
            console.log("No menu data found for the seller");
          }
        } else {
...
<MenuTable heading={'Menu'}/>
      ...
      {menuData && (
        <div className="w-full mt-7">
          <MenuTable heading={'Menu'} menuData={menuData}/> {/* Pass menuData to MenuTable component */}
        </div>
      )}
    </>
  );
